...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, decoder_input_ids, decoder_input_mask, label_ids, position_ids, docid):
...
def find_sub_list(m_tokens, doctext_tokens):
    m_tokens = remove_led_prefix_from_tokens(m_tokens)
    doctext_tokens = remove_led_prefix_from_tokens(doctext_tokens)
    m_len = len(m_tokens)
    for idx, t in enumerate(doctext_tokens): 
        if t == m_tokens[0]:
    ...
    with open(file_path, encoding="utf-8") as f:
        for idx, line in enumerate(f):
            ...
            for role, entitys in extracts_raw.items():
                extracts[role] = []
                for entity in entitys:
                    # Take only 1st mention of entity
                    first_mention_tokens = tokenizer.tokenize(entity[0][0])
                    # Find start and end of mention from doctext tokens, kinda buggy dis function
...
def convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        label_ids = tgt_position_ids[1:] #Used to point back to a position because of the pointer network
        ...
        padding_length = max_seq_length_tgt - len(tgt_tokens_ids) #len(tgt_tokens)
        ...
        encoder_input_ids = src_tokens_ids
        decoder_input_mask = tgt_mask #tgt_tgt_causal_mask
        decoder_input_ids = tgt_tokens_ids
        ...
        if ex_index < 1:
            ...
            logger.info("tokens: %s", " ".join([str(x) for x in tokenizer.convert_tokens_to_ids(tokens)]))
            logger.info("encoder_input_ids: %s", " ".join([str(x) for x in encoder_input_ids]))
            logger.info("encoder_input_mask: %s", " ".join([str(x) for x in encoder_input_mask]))
            logger.info("decoder_input_ids: %s", " ".join([str(x) for x in decoder_input_ids]))
            logger.info("decoder_input_mask: %s", " ".join([str(x) for x in decoder_input_mask]))
            # logger.info("segment_ids: %s", " ".join([str(x) for x in segment_ids]))
            # logger.info("position_ids: %s", " ".join([str(x) for x in position_ids]))
            logger.info("label_ids: %s", " ".join([str(x) for x in label_ids]))

        features.append(
            InputFeatures(input_ids=encoder_input_ids, input_mask=encoder_input_mask, decoder_input_ids=decoder_input_ids, decoder_input_mask=decoder_input_mask, label_ids=label_ids, position_ids=position_ids, docid=docid)
        )
